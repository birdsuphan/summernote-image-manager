...
function filemanagerCreateFolder(){
	...
	f.target = 'updateFrm';
	f.action = summernoteUrl+"/imagemanager/filemanager.php?action=createfolder",
	f.submit();
}
function filemanagerUploadimage(){
	...
	f.enctype = 'multipart/form-data';
	f.target = 'updateFrm';
	f.action = summernoteUrl+"/imagemanager/filemanager.php?action=uploadimage",
	f.submit();
	$('#file_img').val('');
}
...
function deleteImageByName(imgmyname){
	if(confirm('Are you sure to delete.')){
		$.ajax({
		   type: "GET",
		   url:  summernoteUrl+"/imagemanager/filemanager.php?action=deleteimage&path="+imgmyname,
		   success: function(resp){
...
function deletefolder(myfoldername){
	if(confirm('Are you sure to delete.')){
		$.ajax({
		   type: "GET",
		   url:  summernoteUrl+"/imagemanager/filemanager.php?action=deletefolder&path="+myfoldername,
		   success: function(resp){
		   		var path_fd = $('#folder_path').val();
		   		imageFolder2(path_fd);
			}
